%% XEP-0133 "Delete User" Ad-Hoc command handler
%% (http://xmpp.org/extensions/xep-0133.html#delete-user)

-module(mod_adhoc_delete_user).
...
get_local_items(_Host, [], Server, Lang) ->
    {result,
     [?NODE("Delete User (Forced)", ?NS_ADMINX("delete-user"))
     ]};
...
get_local_identity(Acc, _From, _To, Node, Lang) ->
    ...
    case LNode of
    ?NS_ADMINL("delete-user") ->
        ?INFO_COMMAND("Delete User (Forced)", Lang);
    _ ->
        Acc
    end.
...
adhoc_local_commands(From, #jid{lserver = LServer} = _To,
    ...
    XData == false, ActionIsExecute ->
        %% User requests form
        case get_delete_user_form(LServer, Lang) of
        {result, Form} ->
            {stop, adhoc:produce_response(
              Request,
    ...
    XData /= false, ActionIsExecute ->
        ...
        Fields ->
            case catch set_delete_user_form(From, LServer, Lang, Fields) of
            {result, Res} ->
                {stop, adhoc:produce_response(
...
get_delete_user_form(_Host, Lang) ->
    {result, [{xmlelement, "x", [{"xmlns", ?NS_XDATA}, {"type", "form"}],
           [?HFIELD(),
        {xmlelement, "title", [],
         [{xmlcdata, ?T(Lang, "Delete User (Forced)")}]},
            {xmlelement, "field",
         [{"type", "jid-multi"},
...
set_delete_user_form(From, Host, Lang, XData) ->
