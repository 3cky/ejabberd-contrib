...
log_packet(From, To, Packet = {xmlelement, "message", Attrs, Els}, Host) ->
	...
	_ ->
        ...
        true ->
           ?DEBUG("don't log invite message: ~s", [xml:element_to_string(Packet)]),
           ok
        end
    end;
...
write_packet(From, To, Packet, Host) ->
    ...
    case Subject ++ Body of
	    ...
	    {FiledirTemplate, BaseFilename, TimeString, Header, MessageType} =
		case calendar:local_time() of
			...
			{
             lists:flatten(io_lib:format("~s/~~p/~~2.2.0w/~~2.2.0w/", [Path])),
             lists:flatten(io_lib:format("~s - ~s~s", SortedJid++[template(Format, extension)])),
			 io_lib:format(template(Format, time), [H, Min, S]),
			 io_lib:format(template(Format, header),
                           lists:duplicate(count(template(Format, header), "~s"), Title)),
	    ...
	    ?DEBUG("FiledirTemplate ~p~n",[FiledirTemplate]),
	    ?DEBUG("BaseFilename ~p~n",[BaseFilename]),
	    FilenameTemplate = FiledirTemplate++BaseFilename,
        Filedir = io_lib:format(FiledirTemplate, [Y, M, D]),
	    ...
	    File = case file:read_file_info(Filename) of
			   ...
			   NewFile = open_logfile(Filedir, Filename),
	    ...
	    io:format(File, lists:flatten(template(Format, MessageType)),
                  [TimeString, FromJid, From#jid.lresource, ToJid, To#jid.lresource, MessageText]),
	    file:close(File)
    end.
...
open_logfile(Filedir, Filename) ->
    case filelib:ensure_dir(Filedir) of
    ok ->
        open_logfile(Filename);
    {error, Reason} ->
        ?ERROR_MSG("Cannot create intermediate dirs ~s: ~p~n", [Filedir, Reason])
end.
